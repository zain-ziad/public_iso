...
class msgbox(ctk.CTkToplevel):
    def __init__(self,
        ...
        if icon == 'loading':
            ...
            if icon == 'upload':
                icon_image = ImageTk.PhotoImage(Image.open('app\\core\\assets\\upload_msgbox.png').resize((80, 80), Image.Resampling.LANCZOS))
            if icon == 'success':
                icon_image = ImageTk.PhotoImage(Image.open('app\\core\\assets\\success_msgbox.png').resize((80, 80), Image.Resampling.LANCZOS))
            if icon == 'fail':
                icon_image = ImageTk.PhotoImage(Image.open('app\\core\\assets\\failed_msgbox.png').resize((80, 80), Image.Resampling.LANCZOS))
            if icon == 'info':
                icon_image = ImageTk.PhotoImage(Image.open('app\\core\\assets\\info_msgbox.png').resize((80, 80), Image.Resampling.LANCZOS))
            if icon == 'warning':
                icon_image = ImageTk.PhotoImage(Image.open('app\\core\\assets\\warning_msgbox.png').resize((80, 80), Image.Resampling.LANCZOS))
        ...
        # Define the desired and starting pad values for the mainframe
        desired_padx = 56
        desired_pady = 42
        self.starting_padx = desired_padx * 0.75
        self.starting_pady = desired_pady * 0.75
        ...
        if auto_size:
            ...
            def animate():
                ...
                pad_x = self.starting_padx + (desired_padx - self.starting_padx) * (self.current_step / self.animation_steps)
                pad_y = self.starting_pady + (desired_pady - self.starting_pady) * (self.current_step / self.animation_steps)

                # Update the window geometry
                self.geometry(f"{int(step_width)}x{int(step_height)}+{int(current_x)}+{int(current_y)}")
                #self.midframe.pack_configure(padx=pad_x, pady=pad_y)
            ...
            #self.midframe.pack_configure(padx=self.starting_padx, pady=self.starting_pady)
            self.after(1, animate)
