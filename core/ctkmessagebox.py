...
class CTkMessagebox(customtkinter.CTkToplevel):
    ...
    def __init__(self,
                 ...
                 bg_color: str = "#FFFFFF",
                 fg_color: str = "#FFFFFF",
                 text_color: str = "#5F5F7E",
                 title_color: str = "#1C72AE",
        ...
        self.sound = sound
        self.cancel_button = cancel_button if cancel_button else None      
        ...
        self.title_label = customtkinter.CTkLabel(self.frame_top, width=1, text=self._title, text_color=self.title_color, font=('Poppins Semibold' , 24))
        self.title_label.grid(row=0, column=0, columnspan=6, sticky="we", pady=(24, 0))
    ...
    def load_icon(self, icon, icon_size):
        if icon not in self.ICONS or self.ICONS[icon] is None:
            if icon in ["check", "cancel", "info", "question", "warning"]:
                image_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'assets\\ctkmsgbox_icons', icon + '.png')
            else:
                image_path = icon
            if icon_size:
                ...
                size = (int(self.height / 4), int(self.height / 4))
            ico = Image.open(image_path).resize(size)
            self.ICONS[icon] = ImageTk.PhotoImage(ico)
